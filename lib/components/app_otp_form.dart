...
class _AppOtpFormState extends State<AppOtpForm> {
  ...
  bool get _showButton => widget.buttonText != null && widget.onVerify != null;
  ...
  double _otpBoxSize(double availableWidth) {
    final gap = size(widget.codeLength > 4 ? 8 : 12);
    final maxBoxSize = size(56);
    final minBoxSize = size(36);
    final rawSize =
        (availableWidth - gap * (widget.codeLength - 1)) / widget.codeLength;
    return rawSize.clamp(minBoxSize, maxBoxSize);
  }

  double _otpGap(int codeLength) => size(codeLength > 4 ? 8 : 12);

  double _otpFontSize(double boxSize) =>
      widget.textSize ?? size(math.min(22, boxSize * 0.42));
  ...
  Widget _buildOtpField({
    ...
    return Padding(
      padding: EdgeInsets.only(right: index < widget.codeLength - 1 ? gap : 0),
  ...
  Widget build(BuildContext context) {
    ...
    return Skeletonizer(
      enabled: widget.isLoading,
      child: Container(
        width: widget.width ?? size(360),
        padding:
            widget.padding ??
            EdgeInsets.symmetric(horizontal: size(24), vertical: size(32)),
        decoration: BoxDecoration(
          color: widget.backgroundColor ?? uiTheme.surface,
          borderRadius: BorderRadius.circular(size(20)),
          border: isDark
              ? Border.all(color: uiTheme.borderColor, width: 1)
              : null,
          boxShadow: [
            BoxShadow(
              ...
              Skeleton.replace(
                ...
                child: Text(
                  ...
                  style: Theme.of(context).textTheme.titleLarge?.copyWith(
                    fontSize: widget.titleSize ?? size(20),
                    fontWeight: FontWeight.bold,
                    color: uiTheme.onSurface,
                  ),
                ),
            ...
            if (_showButton) ...[
              ...
              Skeleton.leaf(
                child: AppButton(
                  ...
                  onPressed: widget.isLoading ? () {} : widget.onVerify,
                  isMax: true,
                  ...
                  children: [
                    if (widget.footerText != null)
                      Text(
                        widget.footerText!,
                        style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                          fontSize: widget.footerTextSize ?? size(14),
                          color: uiTheme.hintColor,
                        ),
                      ),
